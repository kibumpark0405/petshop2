<template>
    <div style="max-height:80vh;">
        ...
        <div class="gs-bundle-of-buttons" style="max-height:10vh;">
            ...
            <v-btn @click="updateProductDialog = true" class="contrast-primary-text" small color="primary" :disabled="!hasRole('Admin')">
                <v-icon small>mdi-minus-circle-outline</v-icon>상품 수정
            </v-btn>
            <v-dialog v-model="updateProductDialog" width="500">
                <UpdateProductCommand
                    @closeDialog="updateProductDialog = false"
        ...
        <wj-flex-grid
            ...
            class="wj-felx-grid"
        >
            <wj-flex-grid-filter :filterColumns="['RowHeader','name','price','description','ingredients',]" />
            ...
            <wj-flex-grid-column binding="ingredients" header="Ingredients" width="2*" :isReadOnly="true" align="center">
                <wj-flex-grid-cell-template cellType="Cell" v-slot="cell">   
                    <List&lt;Ingredient&gt; v-model="cell.item.ingredients" :editMode="editMode"></List&lt;Ingredient&gt;>
                </wj-flex-grid-cell-template>
            </wj-flex-grid-column>
        </wj-flex-grid>
...
export default {
    ...
    methods:{
        updateProduct(params){
            try{
                this.repository.invoke(this.getSelectedItem(), "updateProduct", params)
                this.$EventBus.$emit('show-success','UpdateProduct 성공적으로 처리되었습니다.')
            }catch(e){
